...
import de.elamx.clt.cutout.CutoutGeometry.Property;
 ...
 * @author raedel
 */
public class LoadSaveCutoutGeometry {
    
    private static final String CUTOUTGEOMETRY_ELEMENT = "CutoutGeometry";
    private static final String NAME_TAG = "name";

    public static void loadCutoutGeometry(Element elem, CutoutInput input) {
        load(elem, input);
    }

    private static void load(Element elem, CutoutInput input) {
        NodeList propList = elem.getElementsByTagName(CUTOUTGEOMETRY_ELEMENT);
        if (propList.getLength() > 0) {

            HashMap<String, CutoutGeometry> serviceMap = new HashMap<>();
            for (CutoutGeometry s: Lookup.getDefault().lookupAll(CutoutGeometry.class)){
            ...
            for (int jj = 0; jj < propList.getLength(); jj++) {
                ...
                if (propNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                    ...
                    CutoutGeometry cg = ((CutoutGeometry)sObject).getCopy();
    ...
    private static String getTagValue(String sTag, Element eElement) {
        ...
        org.w3c.dom.Node nValue = n.getChildNodes().item(0);

        return nValue.getNodeValue();
    }

    public static void storeCutoutGeometry(Document doc, Element dataElement, CutoutInput input) {
        store(doc, dataElement, input.getCutoutGeometry());
    }

    private static void store(Document doc, Element dataElement, CutoutGeometry serv) {

        Element propElem = doc.createElement(CUTOUTGEOMETRY_ELEMENT);
