...
public class ResultWriter {
    ...
    public static void writeResults(FileWriter file, Laminat laminate, PressureVesselInput input, CLT_LayerResult[] results){
        if (file == null) {
            return;
        }
        ...
        for (int ii = 0; ii < layerNum; ii++){
            ...
            for (String key : m.getAdditionalValueKeySet()){
                maxCharacters = Math.max(maxCharacters, m.getAdditionalValueDisplayName(key).length());
            }
            for (String key : m.getAdditionalValueKeySet()){
                out.printf(lo,"  %-" + maxCharacters + "s = %-10.5f%n", m.getAdditionalValueDisplayName(key), m.getAdditionalValue(key));
            }
        }
        ...
        out.println("_______________________________________________________________________________");
        out.println();
        out.println("Pressue load :");
        out.println();
        out.printf(lo,"  nxx  = %-17.10E%n", input.getRadius());
        out.printf(lo,"  nyy  = %-17.10E%n", input.getPressure());
        
        switch (input.getRadiusType()){
            case PressureVesselInput.RADIUSTYPE_INNER:
            out.printf(lo,"  nxy  = %s%n", NbBundle.getMessage(CLT_PressureVesselTopComponent.class, "CLT_PressureVesselTopComponent.radiusTypeBox.item0"));
            break;
            case PressureVesselInput.RADIUSTYPE_MEAN:
            out.printf(lo,"  nxy  = %s%n", NbBundle.getMessage(CLT_PressureVesselTopComponent.class, "CLT_PressureVesselTopComponent.radiusTypeBox.item1"));
            break;
            case PressureVesselInput.RADIUSTYPE_OUTER:
            out.printf(lo,"  nxy  = %s%n", NbBundle.getMessage(CLT_PressureVesselTopComponent.class, "CLT_PressureVesselTopComponent.radiusTypeBox.item2"));
            break;
            default:
            out.printf(lo,"  nxy  = %s%n", NbBundle.getMessage(CLT_PressureVesselTopComponent.class, "CLT_PressureVesselTopComponent.radiusTypeBox.item1"));
        }
