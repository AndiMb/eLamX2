 ...
 * @author Tim Dorau
 */
public class Hoffman extends Criterion {

    public static Hoffman getDefault(FileObject obj) {
        Hoffman hf = new Hoffman(obj);

        return hf;
    }

    public Hoffman(FileObject obj) {
        super(obj);
    }
    ...
    public ReserveFactor getReserveFactor(Material material, Layer l, StressStrainState sss) {
        ...
        if (Q == 0.0 && L == 0.0) {
            ...
            rf.setFailureName(NbBundle.getMessage(Hoffman.class, "Hoffman." + rf.getFailureName()));
        }

        return rf;
    }

    /* Versagenskörper entspricht Tsai-Wu mit den neuen Konstanten F_ij .*/
    @Override
    public Mesh getAsMesh(Material material, double quality) {
        ...
        // Berechnen der Festigkeitsparameter des Hoffman-Kriteriums aus den Festigkeiten
        double F1 = 1 / material.getRParTen() - 1 / material.getRParCom();
        double F2 = 1 / material.getRNorTen() - 1 / material.getRNorCom();
        double F11 = 1 / (material.getRParTen() * material.getRParCom());
        double F22 = 1 / (material.getRNorTen() * material.getRNorCom());
