...
public class CalculationNodeProviderImpl implements eLamXModuleDataNodeProvider{

    @Override
    public eLamXModuleData[] geteLamXModuleData(Laminat laminat) {
        return laminat.getLookup().lookupAll(CalculationModuleData.class).toArray(new CalculationModuleData[0]);
    }

    @Override
    public Node getNodes(eLamXModuleData moduleData) {
        if (moduleData instanceof CalculationModuleData) {
            return new CalculationDataNode((CalculationModuleData)moduleData);
        }
        return null;
    }
