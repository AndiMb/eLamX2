 ...
 * @author maik
 */
public class Puck extends MicroMechModel{
    ...
    public double getE22(Fiber fiber, Matrix matrix, double phi) {
        double volm = 1.0-phi;
        double x = matrix.getNue12();
        x *= x;
        double y = phi;
        y *= phi;
        double z = Math.pow(volm, 1.25);
        return (matrix.getEpar()/(1.0-x))*((1.0+0.85*y)/(z+phi*(matrix.getEpar()/(fiber.getEnor()*(1.0-x)))));
    }
    ...
    public double getG12(Fiber fiber, Matrix matrix, double phi) {
        double volm = 1.0-phi;
        double a = Math.sqrt(phi);
        double b = Math.pow(volm, 1.45);
        return matrix.getG()*(1.0+0.4*a)/(b+phi*(matrix.getG()/fiber.getG()));
    }    
}