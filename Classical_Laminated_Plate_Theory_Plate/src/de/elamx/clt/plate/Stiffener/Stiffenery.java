 ...
 * @author raedel
 */
public class Stiffenery extends Stiffener {

    public Stiffenery(StiffenerProperties properties, double position) {
            ...
            for (int qq = 0; qq < n; qq++) {
                ...
                for (int ii = 0; ii < m; ii++) {
                    for (int jj = 0; jj < n; jj++) {
                        l++;
                        kmat[k][l] += E * I * by.IdX2dX2(jj, qq) * bx.wx(ii, tPos)  * bx.wx(pp, tPos)
                                    + G * J * by.IdXdX(jj, qq)   * bx.wdx(ii, tPos) * bx.wdx(pp, tPos);
                    }
                }
            }
        }
        ...
        for (int pp = 0; pp < m; pp++){
            for (int qq = 0; qq < n; qq++){
                ...
                for (int ii = 0; ii < m; ii++){
                    for (int jj = 0; jj < n; jj++){
                        l++;
                        mmat[k][l] -= rho*A*by.IXX(jj,qq)    *bx.wx(ii,tPos) *bx.wx(pp,tPos);
                    }
                }
            }
        }
    ...
    public int getDirection() {
        return Stiffener.Y_DIRECTION;
    }

    @Override
    public String getDirectionAsString() {
        return Stiffener.Y_DIRECTION_STRING;
    }
